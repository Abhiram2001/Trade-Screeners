...
==========================================
Based on: Requirement2_doc_tradingview.docx

JOB 1 — Weekly Bullish / Daily Pullback Scan
  Universe : S&P 500 + Nasdaq 100
  Conditions:
    • Weekly : Price > Weekly EMA 200   (long-term bullish trend)
    • Daily  : Price < Daily EMA 200    (short-term pullback)
    • Daily  : Price > Daily EMA 8      (early recovery / near-term strength)
  Output : Potential_Bullish2_<date>.html  +  job2_watchlist.json

JOB 2 — Identify 20 EMA Breakout Stocks
  Input  : Symbols from Job 1 watchlist (job2_watchlist.json)
  Condition : Same-day EMA 20 upward crossover — open ≤ EMA 20 AND close > EMA 20
  Output : Bullish2_20ema_<date>.html  (recreated fresh every run)

Usage:
    python3 Screener-2.py          # Run both jobs
    python3 Screener-2.py job1     # Run Job 1 only
    python3 Screener-2.py job2     # Run Job 2 only (requires prior Job 1 run)

Schedule (crontab, weekdays 9 AM):
    0 9 * * 1-5  /path/to/.venv/bin/python /path/to/Screener-2.py >> screener2.log 2>&1
"""
...
WATCHLIST_FILE  = os.path.join(BASE_DIR, "job2_watchlist.json")
...
_JOB1_FILTERS = [
    ...
    # Daily Price < Daily EMA 200 → short-term pullback
    {"left": "close",     "operation": "less",    "right": "EMA200"},
    # Daily Price > Daily EMA 8 → early recovery / near-term strength
    {"left": "EMA8",      "operation": "less",    "right": "close"},
]

def run_job1(universe: set, sp500_n: int, ndx_n: int) -> list[dict]:
    print("\n" + "─" * 60)
    print("  JOB 1 — Weekly Bullish / Daily Pullback Stocks")
    print("─" * 60)
    print("  Conditions:")
    print("    ✓ Price > EMA 200 (Weekly) — long-term bullish trend")
    print("    ✓ Price < EMA 200 (Daily)  — short-term pullback")
    print("    ✓ Price > EMA 8  (Daily)   — early signs of recovery")
    ...
    for item in raw_rows:
        ...
        try:
            ...
            if symbol not in universe:
                continue
            if not price or not ema200d or not ema200w or not ema8:
                continue

            perf_w = d[6]
            mktcap = d[9]

            gap_ema200d_pct = round((price - ema200d) / ema200d * 100, 2)  # negative = below
            gap_ema8_pct    = round((price - ema8)    / ema8    * 100, 2)  # positive = above

            results.append({
                "Symbol":        symbol,
                "Company":       (d[1] or "")[:30],
                "Price":         round(price, 2),
                "EMA8_1D":       round(ema8, 2),
                "Gap_EMA8%":     gap_ema8_pct,
                "EMA200_1D":     round(ema200d, 2),
                "Gap_EMA200D%":  gap_ema200d_pct,
                "EMA200_1W":     round(ema200w, 2),
    ...
    _print_console(results, "JOB 1 — Potential_Bullish2",
                   ["Symbol","Company","Price","EMA8_1D","Gap_EMA8%","EMA200_1D","Gap_EMA200D%","EMA200_1W","Perf_W%","RSI_1D","Change%","Day_High","Day_Low","Week_High","Week_Low","Sector"])
    _save_html(results, job_name="Potential_Bullish2", job_num=1,
               universe_label=f"S&P 500 ({sp500_n}) + Nasdaq 100 ({ndx_n})",
               conditions=[
                   "Price > EMA 200 Weekly  (long-term bullish trend)",
                   "Price < EMA 200 Daily   (short-term pullback)",
                   "Price > EMA 8 Daily     (early signs of recovery)",
               ])
    return results
...
_JOB2_COLUMNS = [
    ...
    "EMA200",           # 5  EMA 200 Daily
    "EMA200|1W",        # 6  EMA 200 Weekly
    "EMA8",             # 7  EMA 8 Daily
    "RSI",              # 8
    "volume",           # 9
    "market_cap_basic", # 10
    "sector",           # 11
    "change",           # 12
    "high",             # 13 Last market day High
    "low",              # 14 Last market day Low
    "high|1W",          # 15 Last week High
    "low|1W",           # 16 Last week Low
]

# Crossover: close crossed EMA20 today → filter to upward cross client-side
_JOB2_FILTERS = [
...
def run_job2(universe: set, sp500_n: int, ndx_n: int) -> list[dict]:
    ...
    if not os.path.exists(WATCHLIST_FILE):
        print("  [error] job2_watchlist.json not found. Run Job 1 first.")
        return []
    ...
    for item in raw_rows:
        ...
        try:
            ...
            results.append({
                ...
                "Week_High":   round(d[15], 2) if d[15] else None,
                "Week_Low":    round(d[16], 2) if d[16] else None,
            })
        except Exception:
            continue
    ...
    _print_console(results, "JOB 2 — Bullish2_20ema",
                   ["Symbol","Company","Price","Open","EMA20_1D","Gap_EMA20%","EMA8_1D","EMA200_1D","RSI_1D","Change%","Day_High","Day_Low","Week_High","Week_Low","Sector"])
    _save_html(results, job_name="Bullish2_20ema", job_num=2,
               universe_label=f"Job 1 watchlist ({len(job1_symbols)} symbols)",
               conditions=[
                   "Stock was in Job 1 Potential_Bullish2 watchlist",
...
def _save_html(rows: list[dict], job_name: str, job_num: int,
    ...
    job_color   = "#f59e0b" if job_num == 1 else "#a78bfa"   # amber for J1, violet for J2
    job_label   = "Weekly Bullish / Daily Pullback" if job_num == 1 else "20 EMA Breakout"
    condition_html = "".join(f'<li>{c}</li>' for c in conditions)

    if not rows:
        cols = []
    elif job_num == 1:
        cols = ["Symbol","Company","Price","EMA8_1D","Gap_EMA8%","EMA200_1D","Gap_EMA200D%","EMA200_1W","Perf_W%","RSI_1D","Change%","Day_High","Day_Low","Week_High","Week_Low","MarketCap_B","Sector"]
    else:
        cols = ["Symbol","Company","Price","Open","EMA20_1D","Gap_EMA20%","EMA8_1D","EMA200_1D","EMA200_1W","RSI_1D","Change%","Day_High","Day_Low","Week_High","Week_Low","MarketCap_B","Sector"]
    ...
    col_headers = {
        ...
        "EMA8_1D": "EMA 8 (1D)",
        "Gap_EMA8%": "Gap EMA8%",
        "EMA20_1D": "EMA 20 (1D)", "Gap_EMA20%": "Gap EMA20%",
        "EMA200_1D": "EMA 200 (1D)", "Gap_EMA200D%": "Gap EMA200D%",
        "EMA200_1W": "EMA 200 (1W)",
    ...
    right_align = {"Price","Open","EMA8_1D","Gap_EMA8%","EMA20_1D","Gap_EMA20%","EMA200_1D","Gap_EMA200D%","EMA200_1W","Perf_W%","RSI_1D","Change%","Day_High","Day_Low","Week_High","Week_Low","MarketCap_B"}
        ...
        for c in cols:
            ...
            if c == "Symbol":
                ...
                try:
                    ...
                    display = "—"
            elif c in ("Gap_EMA8%", "Gap_EMA20%"):
                try:
                    v = float(val)
                    display = f"+{v:.2f}%" if v >= 0 else f"{v:.2f}%"
                    cls = "r gap-pos" if v >= 0 else "r gap-neg"
                except (TypeError, ValueError):
                    display = "—"
            elif c == "Gap_EMA200D%":
                try:
                    v = float(val)
                    # Negative means below EMA200 (pullback), expected for Job1
                    display = f"+{v:.2f}%" if v >= 0 else f"{v:.2f}%"
                    cls = "r gap-neg" if v < 0 else "r gap-pos"
                except (TypeError, ValueError):
                    display = "—"
            elif c == "Perf_W%":
                try:
                ...
                try:
                    ...
                    display = "—"
            elif c in ("Price","Open","EMA8_1D","EMA20_1D","EMA200_1D","EMA200_1W","Day_High","Day_Low","Week_High","Week_Low"):
                try:
...
<style>
  ...
  * {{ box-sizing:border-box; margin:0; padding:0; }}
  ...
  td.gap-pos {{ color:var(--green) !important; }}
  td.gap-neg {{ color:var(--red)   !important; }}
...
def main():
    ...
    if mode not in ("job1", "job2", "both"):
        print(f"Usage: python3 Screener-2.py [job1|job2|both]")
        sys.exit(1)

    print("=" * 60)
    print("  TradingView Automated Watchlist Screener — v2")
